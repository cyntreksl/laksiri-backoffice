...
<template>
    <ConfirmationModal :show="show">
        <template #title>
            {{hblData[11]?.data ? 'Release' : 'Hold'}} {{ hblData[1]?.data }} ?
        </template>

        <template #content>
            Would you like to {{hblData[11]?.data ? 'release' : 'hold'}}?
        </template>
        ...
        <template #footer>
            <div class="flex space-x-2">
                ...
                <PrimaryButton v-if="hblData[11]?.data" type="button" @click="$emit('toggleHold')">
