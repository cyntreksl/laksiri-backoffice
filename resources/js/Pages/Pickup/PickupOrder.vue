...
const handleSearch = () => {
    ...
    router.get(route('pickups.ordering'), form, {
        ...
        preserveState: true,
        onSuccess: () => {
            router.visit(route('pickups.ordering', {
...
const handleMoveUp = (index) => {
    if (index > 0) {
        [localPickups.value[index - 1], localPickups.value[index]] = [localPickups.value[index], localPickups.value[index - 1]];
        checkOrderChange();
    } else {
...
const handleSave = () => {
    ...
    router.put(route('pickups.update-order'), {pickups: updatedPickups}, {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            ...
            <div>
                ...
                <div v-if="localPickups.length > 0" class="card mx-5 mb-5">
                    ...
                    <div class="flex my-5 space-x-4 items-center">
                        <PrimaryButton :disabled="!hasOrderChanged" @click.prevent="handleSave">
                            Save Order
                            <svg class="w-6 h-6 ml-3" fill="none" stroke="currentColor" stroke-width="1.5"
                                 viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M3 7.5 7.5 3m0 0L12 7.5M7.5 3v13.5m13.5 0L16.5 21m0 0L12 16.5m4.5 4.5V7.5" stroke-linecap="round"
                                      stroke-linejoin="round"/>
                            </svg>
                        </PrimaryButton>
                        <p class="text-error">
                            Total Records {{ localPickups.length }}
                        </p>
                    </div>
                                ...
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    ...
                                    >
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5"
                                             viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M4.5 10.5 12 3m0 0 7.5 7.5M12 3v18" stroke-linecap="round"
                                                  stroke-linejoin="round"/>
                                        </svg>
                                    </button>
                                    <button
