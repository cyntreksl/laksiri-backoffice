            ...
            then: (data) =>
                data.data.map((item) => [
                    ...
                    h(
                        ...
                        {
                            class:
                                "tag rounded-full bg-success/10 text-success hover:bg-success/20 focus:bg-success/20 active:bg-success/25 mr-1",
                        },
            ...
            return h("div", {}, [
                ...
                h(
                    "button",
                    {
                        ...
                        onClick: () => confirmDriverAreas(row.cells[0].data),
                    },
                    [
                        h(
                            "svg",
...
const showConfirmDriverAreasModal = ref(false);
const DriverAreasid = ref(null);

const confirmDriverAreas = (id) => {
    DriverAreasid.value = id;
    showConfirmDriverAreasModal.value = true;
};

const closeModal = () => {
    showConfirmDriverAreasModal.value = false;
};

const handleDeleteDriverAreas = () => {
    router.delete(route("setting.driver-areas.destroy", DriverAreasid.value), {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
...
<template>
    <AppLayout title="Driver Areas">
        ...
        <CreateDriverAreasForm :zones="zones"/>
        ...
        <DeleteDriverAreasConfirmationModal
            :show="showConfirmDriverAreasModal"
            @close="closeModal"
            @delete-DriverAreas="handleDeleteDriverAreas"
        />
    </AppLayout>
</template>
