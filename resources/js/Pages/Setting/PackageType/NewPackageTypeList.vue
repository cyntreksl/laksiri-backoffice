...
import CreatePackageTypeForm from "@/Pages/Setting/PackageType/CreatePackageTypeForm.vue";
import DeletePackageTypeConfirmationModal
    from "@/Pages/Setting/PackageType/Partials/DeletePackageTypeConfirmationModal.vue";
...
const showConfirmDeletePackageTypeModal = ref(false);
const packageTypeId = ref(null);

const confirmDeletePackageType = (id) => {
    packageTypeId.value = id;
    showConfirmDeletePackageTypeModal.value = true;
};

const closeModal = () => {
    showConfirmDeletePackageTypeModal.value = false;
    packageTypeId.value = null;
};

const handleDeletePackageType = () => {
    router.delete(route("setting.package-types.destroy", packageTypeId.value), {
        preserveScroll: true,
        onSuccess: () => {
            closeModal();
    ...
    <AppLayout title="Package Types">
        ...
        <div class="min-w-full overflow-auto">
            <table class="is-hoverable w-full text-left">
                ...
                <tbody>
                <tr
                    v-for="(packageType, index) in packageTypes"
                    v-if="packageTypes && Object.keys(packageTypes).length > 0"
                    :key="packageType.id"
                ...
                >
                    ...
                    <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                        <div class="flex space-x-2">
                            <Link
                                :href="route('setting.package-types.edit', packageType.id)"
                            ...
                            <button
                                ...
                                @click="confirmDeletePackageType(packageType.id)"
                            >
                                <svg
                                    class="size-4.5"
                                    fill="none"
        ...
        <DeletePackageTypeConfirmationModal :show="showConfirmDeletePackageTypeModal"
                                            @close="closeModal" @delete-package-type="handleDeletePackageType"/>
    </AppLayout>
</template>
