...
const fetchDuePayments = async (page = 1, search = "", sortField = 'created_at', sortOrder = 0) => {
    loading.value = true;
    try {
...
const debouncedFetchHBLs = debounce((searchValue) => {
    fetchDuePayments(1, searchValue);
}, 1000);
...
const onSort = (event) => {
    fetchDuePayments(currentPage.value, filters.value.global.value, event.sortField, event.sortOrder);
};
