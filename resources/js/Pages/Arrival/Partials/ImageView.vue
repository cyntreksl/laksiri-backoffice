...
const fetchImages = async () => {
    if (!props.imageIds || props.imageIds.length === 0) return;

    isLoading.value = true;
    try {
        const responses = await Promise.all(
            props.imageIds.map(async (id) => {
                ...
                return data.imageUrl; // Assuming the API returns an object with `imageUrl`
            })
        );

        images.value = responses;
    } catch (error) {
        ...
        <template #content>
            <div v-if="isLoading" class="flex justify-center items-center h-48">
                <svg class="animate-spin h-8 w-8 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 0012 20c4.411 0 8-3.589 8-8H4c0 2.206 1.794 4.109 4 4.709z"></path>
                </svg>
            </div>

            <div v-else-if="images.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
