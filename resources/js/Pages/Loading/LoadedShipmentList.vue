...
const createColumns = () => [
    ...
    {name: 'Maximum Volumetric Weight', hidden: !data.columnVisibility.maximum_volumetric_weight},
    {name: 'Minimum Volumetric Weight', hidden: !data.columnVisibility.minimum_volumetric_weight},
    ...
    {name: 'Departure Airport', hidden: !data.columnVisibility.airport_of_departure},
    {name: 'Arrival Airport', hidden: !data.columnVisibility.airport_of_arrival},
    {name: 'Cargo Class', hidden: !data.columnVisibility.cargo_class},
    {name: 'Status', hidden: !data.columnVisibility.status},
    {name: 'Loading Started At', hidden: !data.columnVisibility.loading_started_at},
    {name: 'Loading Ended At', hidden: !data.columnVisibility.loading_ended_at},
    {name: 'Unloading Started At', hidden: !data.columnVisibility.unloading_started_at},
    {name: 'Unloading Ended At', hidden: !data.columnVisibility.unloading_ended_at},
    {name: 'Loading Started By', hidden: !data.columnVisibility.loading_started_by},
    {name: 'Loading Ended By', hidden: !data.columnVisibility.loading_ended_by},
    {name: 'Unloading Started By', hidden: !data.columnVisibility.unloading_started_by},
    {name: 'Unloading Ended By', hidden: !data.columnVisibility.unloading_ended_by},
    {
        name: 'Actions',
        sort: false,
        ...
        formatter: (_, row) => {
            return h('div', {}, [
                h('a', {
                    href: route('loading.loaded-containers.manifest.export', row.cells[0].data)
                }, [
                    h('button', {
                        className: 'btn size-8 p-0 text-warning hover:bg-warning/20 focus:bg-warning/20 active:bg-warning/25',
                        'x-tooltip..placement.bottom.warning': "'Download Manifest'"
                    }, [
                        h('svg', {
                            xmlns: 'http://www.w3.org/2000/svg',
                            viewBox: '0 0 24 24',
                            class: 'size-6 icon icon-tabler icons-tabler-outline icon-tabler-file-download',
                            fill: 'none',
                            stroke: "currentColor",
                            strokeWidth: 2,
                            strokeLinecap: "round",
        ...
        <FilterDrawer :show="showFilters" @close="showFilters = false">
            ...
            <template #content>
                ...
                <div>
                    ...
                    <DatePicker v-model="filters.etdStartDate" placeholder="Choose date..."/>
                </div>

                <div>
                    <InputLabel value="ETD End Date"/>
                    <DatePicker v-model="filters.etdEndDate" placeholder="Choose date..."/>
                </div>
                ...
                <label v-for="cargoType in cargoTypes" :key="cargoType" class="inline-flex items-center space-x-2 mt-2">
                    <Switch v-model="filters.cargoType" :label="cargoType" :value="cargoType"/>
                </label>
                ...
                <label v-for="containerType in containerTypes" :key="containerType" class="inline-flex items-center space-x-2 mt-2">
                    <Switch v-model="filters.containerType" :label="containerType" :value="containerType"/>
                </label>
