...
const emit = defineEmits(['close', 'fetchContainer']);
...
<template>
    <DialogModal :closeable="true" :maxWidth="'4xl'" :show="show" @close="close">
        ...
        <template #content>
            <div class="flex">
                <div class="w-full grid grid-cols-4 gap-2 items-center">
                    <div class="col-span-3 w-full">
                        <TextInput class="w-full" v-model="mhblRef" placeholder="Enter MHBL Reference"/>
                    </div>
                    <PrimaryButton :disabled="!mhblRef" class="p-3" @click.prevent="getMHBLWithHBLS">Confirm</PrimaryButton>
                    <div class="col-span-4 w-full">
                        <InputError class="w-full" :message="errorMessage"/>
                    </div>
                </div>
            </div>
            <div v-if="mhblData" class="is-scrollbar-hidden relative space-y-2.5 overflow-y-auto p-0.5 mt-5">
                <div v-if="mhblData.hbls.length > 0" v-for="(hbl, index) in mhblData.hbls" class="card cursor-pointer border shadow-sm">
                    <div class="flex justify-between items-center">
            ...
            <div class="flex">
                <PrimaryButton :disabled="!mhblData" class="w-full mt-2" @click.prevent="handleLoad">Load MHBL</PrimaryButton>
            </div>
        </template>
    </DialogModal>
</template>
