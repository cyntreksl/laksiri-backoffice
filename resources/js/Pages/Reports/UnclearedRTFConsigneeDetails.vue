<template>
    <AppLayout title="Uncleared RTF Consignee Details">
        <template #header>Uncleared RTF Consignee Details</template>

        <div class="uncleared-rtf-page">
            <!-- Page Header -->
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        <i class="ti ti-alert-triangle text-4xl text-primary"></i>
                        <div>
                            <h1 class="text-3xl font-bold text-gray-800">Uncleared RTF Consignee Details</h1>
                            <p class="text-gray-600 mt-1">RTF consignee details for {{ reportDate }}</p>
                        </div>
                    </div>
                    ...
                    <template #content>
                        <div class="card-content">
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-purple-600">{{ formatCBM(stats.total_cbm) }}</p>
                            </div>
                        </div>
                    </template>
                </Card>
                    ...
                    <div class="filters-section">
                        <div class="filters-grid">
                            ...
                            <FloatLabel class="w-full" variant="in">
                                ...
                                />
                                <label for="search">Search (HBL/Consignee/Agent)</label>
                            </FloatLabel>
                        </div>
                    ...
                    >
                        <template #empty>
                            <div class="empty-state">
                                ...
                                <p class="text-gray-500 mt-4">No RTF consignee records found</p>
                            </div>
                        </template>

                        <template #loading> Loading RTF consignee data. Please wait.</template>

                        <Column field="serial_no" frozen style="min-width: 100px">
                            <template #body="{ data }">
                                <span class="font-mono font-semibold">{{ data.serial_no }}</span>
                            </template>
                            <template #header>
                        ...
                        <Column field="consignee_name" style="min-width: 200px">
                            <template #body="{ data }">
                        ...
                        <Column field="date" sortable style="min-width: 120px">
                            <template #body="{ data }">
                                <span class="font-mono">{{ data.date ? moment(data.date).format('DD/MM/YYYY') : '' }}</span>
                            </template>
                            <template #header>
                        ...
                        <template #footer>
                            <div class="flex justify-between items-center">
                                <span>In total there are {{ totalRecords }} RTF consignee records.</span>
                                <span class="font-semibold text-purple-600">Total CBM: {{ formatCBM(stats.total_cbm) }}</span>
                            </div>
                        </template>
                    </DataTable>
                </template>
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get(route('report.uncleared-rtf-consignee.data'), { params });
        ...
        console.error('Error fetching uncleared RTF consignee details:', error);
        push.error('Failed to load uncleared RTF consignee details data');
    } finally {
        loading.value = false;
    }
};
...
const onSort = (event) => {
    lazyParams.sort_field = event.sortField || 'date';
...
const exportData = (format = 'xlsx') => {
    ...
    window.location.href = route('report.uncleared-rtf-consignee.export') + '?' + queryString;
