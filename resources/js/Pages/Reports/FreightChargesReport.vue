...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get(route('report.freight-charges.data'), { params });
        ...
        console.error('Error fetching freight charges report:', error);
        push.error('Failed to load freight charges report data');
    } finally {
        loading.value = false;
    }
};
...
const exportData = (format = 'xlsx') => {
    ...
    window.location.href = route('report.freight-charges.export') + '?' + queryString;
...
const exportCSVFilename = computed(() => {
    ...
    return `freight-charges-report-${timestamp}`;
});
...
<template>
    <AppLayout title="Freight Charges Report">
        <template #header>Freight Charges Report</template>

        <div class="freight-charges-report-page">
            <!-- Page Header -->
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        <i class="ti ti-truck-delivery text-4xl text-primary"></i>
                        <div>
                            <h1 class="text-3xl font-bold text-gray-800">Freight Charges Report</h1>
                            <p class="text-gray-600 mt-1">Freight charges for {{ reportDate }}</p>
                        </div>
                    </div>
                    ...
                    <div class="filters-section">
                        <div class="filters-grid">
                            ...
                            <FloatLabel class="w-full" variant="in">
                                ...
                                />
                                <label for="search">Search (HBL, Agent, Consignee)</label>
                            </FloatLabel>
                        </div>
                    ...
                    >
                        ...
                        <template #empty>
                            <div class="empty-state">
                                ...
                                <p class="text-gray-500 mt-4">No freight charges records found</p>
                            </div>
                        </template>

                        <template #loading> Loading freight charges data. Please wait.</template>
                        ...
                        <Column field="hbl_no" header="HBL No" sortable style="min-width: 150px">
                            <template #body="{ data }">
                                <span class="font-mono font-semibold">{{ data.hbl_no }}</span>
                            </template>
                        </Column>

                        <Column field="agent_name" header="Agent Name" sortable style="min-width: 200px">
                            <template #body="{ data }">
                                <span>{{ data.agent_name }}</span>
                            </template>
                        </Column>

                        <Column field="invoice_no" header="Invoice No" sortable style="min-width: 180px">
                            <template #body="{ data }">
                                <span class="font-mono">{{ data.invoice_no || '-' }}</span>
                            </template>
                        </Column>

                        <Column field="consignee_name" header="Consignee Name" sortable style="min-width: 200px">
                            <template #body="{ data }">
                        ...
                        <template #footer>
                            <div class="flex justify-between items-center">
                                <span>In total there are {{ totalRecords }} freight charges records.</span>
