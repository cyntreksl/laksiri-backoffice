...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get(route('report.agent-wise-income.data'), { params });
        ...
        console.error('Error fetching agent wise income analysis:', error);
        push.error('Failed to load agent wise income analysis data');
    } finally {
        loading.value = false;
    }
};
...
const exportData = (format = 'xlsx') => {
    ...
    window.location.href = route('report.agent-wise-income.export') + '?' + queryString;
...
<template>
    <AppLayout title="Agent Wise Income Analysis">
        <template #header>Agent Wise Income Analysis</template>

        <div class="agent-wise-income-page">
            <!-- Page Header -->
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        <i class="ti ti-building text-4xl text-primary"></i>
                        <div>
                            <h1 class="text-3xl font-bold text-gray-800">Agent Wise Income Analysis</h1>
                            <p class="text-gray-600 mt-1">Income analysis by agent for {{ reportDate }}</p>
                        </div>
                    </div>
            ...
            <Card class="my-5">
                <template #content>
                    ...
                    >
                        ...
                        <Column field="agent_name" frozen sortable style="min-width: 250px">
                            <template #body="{ data }">
                        ...
                        <Column field="cbm" style="min-width: 100px">
                            <template #body="{ data }">
                                <span class="font-mono text-right block">{{ formatNumber(data.cbm) }}</span>
                            </template>
                            <template #header>
