...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get(route('report.agent-wise-consignee-volume.data'), { params });
        ...
        console.error('Error fetching agent wise consignee volume analysis:', error);
        push.error('Failed to load agent wise consignee volume analysis data');
    } finally {
        loading.value = false;
    }
};
...
const exportData = (format = 'xlsx') => {
    ...
    window.location.href = route('report.agent-wise-consignee-volume.export') + '?' + queryString;
...
<template>
    <AppLayout title="Agent Wise Consignee & Volume Analysis">
        <template #header>Agent Wise Consignee & Volume Analysis</template>

        <div class="agent-consignee-volume-page">
            <!-- Page Header -->
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-gray-800">Agent Wise Consignee & Volume Analysis</h1>
                            <p class="text-gray-600 mt-1">Total incoming consignee & volume analysis for {{ reportDate }}</p>
                        </div>
                    </div>
            ...
            <div class="summary-cards">
                    ...
                    <template #content>
                        <div class="card-content">
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-purple-600">{{ stats.total_consignees }}</p>
                            </div>
                        </div>
                    </template>
                </Card>
                    ...
                    <template #content>
                        <div class="card-content">
                            ...
                            <div>
                                ...
                                <p class="text-3xl font-bold text-orange-600">{{ stats.total_pkgs_actual }}</p>
                            </div>
                        </div>
                    </template>
                </Card>
                ...
                <Card class="summary-card">
                    <template #content>
                        <div class="card-content">
                            <i class="ti ti-cube text-3xl text-green-500"></i>
                            <div>
                                <p class="text-sm text-gray-500 uppercase">Total CBM</p>
                                <p class="text-3xl font-bold text-green-600">{{ formatNumber(stats.total_cbm) }}</p>
                            </div>
                        </div>
                    </template>
                </Card>
            ...
            <Card class="my-5">
                <template #content>
                    ...
                    >
                        ...
                        <Column field="agent_name" frozen sortable style="min-width: 300px">
                            <template #body="{ data }">
                        ...
                        <Column field="no_of_consignees" style="min-width: 150px">
                            <template #body="{ data }">
                                <span class="text-right block">{{ data.no_of_consignees }}</span>
                            </template>
                            <template #header>
                                <div class="text-right w-full">No of Consignees</div>
                            </template>
                        </Column>

                        <Column field="no_of_pkgs_manifest" style="min-width: 180px">
                            <template #body="{ data }">
                                <span class="text-right block">{{ data.no_of_pkgs_manifest }}</span>
                            </template>
                            <template #header>
                                <div class="text-right w-full">No of PKgs (Manifest)</div>
                            </template>
                        </Column>

                        <Column field="no_of_pkgs_actual" style="min-width: 180px">
                            <template #body="{ data }">
                                <span class="text-right block">{{ data.no_of_pkgs_actual }}</span>
                            </template>
                            <template #header>
                                <div class="text-right w-full">No of PKgs (Actual)</div>
                            </template>
                        </Column>

                        <Column field="cbm" style="min-width: 120px">
                            <template #body="{ data }">
                                <span class="font-mono text-right block font-semibold text-green-600">{{ formatNumber(data.cbm) }}</span>
                            </template>
                            <template #header>
                        ...
                        <template #footer>
                            <div class="flex justify-between items-center">
                                ...
                                <span class="font-semibold text-green-600">Total CBM: {{ formatNumber(stats.total_cbm) }}</span>
                            </div>
                        </template>
                    </DataTable>
                </template>
