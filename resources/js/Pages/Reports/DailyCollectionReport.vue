...
const fetchData = async () => {
    ...
    try {
        ...
        if (filters.date) {
            params.date = moment(filters.date).format('YYYY-MM-DD');
        }
        ...
        const response = await axios.get(route('report.daily-collection.data'), { params });
        ...
        push.error('Failed to load daily collection report data');
    } finally {
        loading.value = false;
    }
};
...
const exportData = (format = 'xlsx') => {
    ...
    window.location.href = route('report.daily-collection.export') + '?' + queryString;
...
<template>
    <AppLayout title="Daily Collection Report">
        ...
        <div class="daily-collection-report-page">
            <!-- Page Header -->
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-gray-800">Daily Collection Report</h1>
                            <p class="text-gray-600 mt-1">Collection record for {{ reportDate }}</p>
                        </div>
                    </div>
            ...
            <div class="summary-cards">
                ...
                <Card class="summary-card">
                    <template #content>
                        <div class="card-content">
                            <i class="ti ti-receipt-tax text-3xl text-purple-500"></i>
                            <div>
                                <p class="text-sm text-gray-500 uppercase">Total VAT</p>
                                <p class="text-3xl font-bold text-purple-600">{{ formatNumber(stats.total_vat) }}</p>
                            </div>
                        </div>
                    </template>
                </Card>
                ...
                <Card class="summary-card">
                    <template #content>
                        <div class="card-content">
                            <i class="ti ti-receipt-tax text-3xl text-orange-500"></i>
                            <div>
                                <p class="text-sm text-gray-500 uppercase">Total NBT</p>
                                <p class="text-3xl font-bold text-orange-600">{{ formatNumber(stats.total_nbt) }}</p>
                            </div>
                        </div>
                    </template>
                </Card>
                        ...
                        <div class="filters-grid">
                            ...
                            <FloatLabel class="w-full" variant="in">
                                <Calendar
                                    ...
                                    showIcon
                                />
                                <label for="date-from">Date From (Optional)</label>
                            </FloatLabel>
                            ...
                            <FloatLabel class="w-full" variant="in">
                                <Calendar
                                    ...
                                    showIcon
                                />
                                <label for="date-to">Date To (Optional)</label>
                            </FloatLabel>
                    ...
                    >
                        ...
                        <template #empty>
                            <div class="empty-state">
                                ...
                                <p class="text-gray-500 mt-4">No collection records found</p>
                            </div>
                        </template>

                        <template #loading> Loading daily collection data. Please wait.</template>

                        <Column field="serial_no" header="Serial No" style="min-width: 100px">
                            <template #body="{ data }">
                                <span class="font-mono">{{ data.serial_no }}</span>
                            </template>
                        </Column>
                        ...
                        <template #footer>
                            <div class="flex justify-between items-center">
                                <span>In total there are {{ totalRecords }} collection records.</span>
                                <div class="flex gap-4">
                                    <span class="font-semibold">Total VAT: {{ formatNumber(stats.total_vat) }}</span>
                                    <span class="font-semibold">Total NBT: {{ formatNumber(stats.total_nbt) }}</span>
