...
<template>
    <AppLayout :title="`${userData}'s HBL`">
        <template #header>  {{userData}} 's HBL</template>
                ...
                <template #content>
                    ...
                    <DataTable
                        ...
                        <Column field="is_hold" header="Hold">
                            <template #body="{ data }">
                                <i :class="{ 'pi-pause-circle text-yellow-500': data.is_hold, 'pi-play-circle text-green-400': !data.is_hold }"
                                   class="pi"></i>
                            </template>
                            <template #filter="{ filterModel, filterCallback }">
                                <div class="flex items-center gap-2">
                                    <Checkbox v-model="filterModel.value" :indeterminate="filterModel.value === null"
                                              binary inputId="is-hold"/>
