...
<template>
    <DestinationAppLayout title="Queue List">
        ...
        <div v-if="Object.keys(filteredExaminationQueue).length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 my-5">
            <Link v-for="queue in filteredExaminationQueue" :key="queue.id" :href="route('call-center.examination.create', queue.id)" class="card grow cursor-pointer hover:bg-indigo-300 items-center p-4 text-center sm:p-5 border w-60 rounded-lg">
                <div class="my-5">
