...
const dt = ref();
const fromDate = ref(moment(new Date()).subtract(6, "months").toISOString().split("T")[0]);
...
const clearFilter = () => {
    ...
    fromDate.value = moment(new Date()).subtract(6, "months").toISOString().split("T")[0];
...
const handleConfirmDriverRemove = (hblId) => {
    confirm.require({
        message: 'Are you certain you want to unassign the driver from this door to door hbl?',
        ...
        accept: () => {
            router.put(route("delivery.driver.unassign", hblId), {
                preserveScroll: true,
                onSuccess: () => {
            ...
            <Card class="my-5">
                <template #content>
                    ...
                    <DataTable
                        ...
                        <template #empty> No Door to Door HBLs found. </template>

                        <template #loading> Loading Door to Door HBLs data. Please wait.</template>

                        <Column headerStyle="width: 3rem" selection-mode="multiple">
                            <template #body="{ data }">
                                <div class="text-center">
                                    <template v-if="data.is_hold === 0 && data.status === 'reached'">
                                        <Checkbox v-model="selectedHBLs" :value="data" />
                                    </template>

                                    <template v-else>
                                        <Button v-tooltip="'Not Reached Yet.'" icon="pi pi-info-circle" rounded severity="warn" size="small" variant="text" />
                                    </template>
                                </div>
                            </template>
                        </Column>
                        ...
                        <template #footer> In total there are {{ hbls ? totalRecords : 0 }} Door to Door HBLs. </template>
                    </DataTable>
                </template>
            </Card>
        </div>
