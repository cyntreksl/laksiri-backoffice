...
<template>
    <AppLayout title="Queue List">
        ...
        <DataView :layout="layout" :value="filteredCashierQueue" class="my-5">
            <template #header>
            ...
            <template #grid="slotProps">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5 my-5 p-5">
                    <Card v-for="queue in slotProps.items" :key="queue.id"
                          class="hover:cursor-pointer hover:bg-emerald-100 !shadow-none border" @click.prevent="() => router.visit(route('call-center.cashier.create', queue.id))">
                        <template #content>
