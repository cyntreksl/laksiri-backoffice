...
@section('pdf-content')
                ...
                <td style="vertical-align: top; width: 60%">
                    <div>
                        ...
                        <div style="text-align: center; line-height: 1">
                            ...
                            <p style="margin: 0; padding: 0; font-family: 'Roboto', sans-serif; color: #636363">NO. 31, ST.
                                ANTHONY'S MW,
                                COLOMBO-03 <br>
    ...
    <table style="margin-top: 0; margin-bottom: -16px; width: 100%;">
        <tr>
    ...
    @php
        $packagesChunks = collect();
        $firstPagePackages = $hbl->packages->take(29); // Take first 29 packages for the first page
        $remainingPackages = $hbl->packages->skip(29); // Skip first 29 packages for remaining pages

        // Add first chunk of 29 packages
        $packagesChunks->push($firstPagePackages);

        // Split remaining packages into chunks of 10
        $packagesChunks = $packagesChunks->merge($remainingPackages->chunk(52));
    ...
    @foreach ($packagesChunks as $chunk)
        <table class="package-table">
            ...
            {{-- Add empty rows if it's the first page and less than 50 packages --}}
            @if ($loop->first && $chunk->count() > 13)
                @for ($i = 0; $i < 29 - $chunk->count(); $i++)
                    <tr>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                ...
                @endfor
            @endif
            {{-- Add empty rows if it's the last page and less than 50 packages --}}
            @if ($loop->last && $chunk->count() > 36)
                @for ($i = 0; $i < 51 - $chunk->count(); $i++)
                    <tr>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
        ...
        @if (!$loop->last || ($loop->last && $chunk->count() > 36))
