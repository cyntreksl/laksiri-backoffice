...
{{--    </div>--}}
    ...
    <!-- Document Info -->
    <div class="grid grid-cols-1 gap-2 text-sm mt-16">
    ...
    <table class="w-full text-sm font-bold border-b">
        <tr>
            ...
            <td colspan="2" class="py-1 w-2/4">
                @if(!empty($containers) && count($containers) > 0)
                    ({{ $containers[0]->container_type }}) {{ $containers[0]->reference }} FCL CONTAINER
                @endif
            </td>
        </tr>
        @foreach($containers->slice(1) as $con)
            <tr>
                <td class="py-1 w-1/4">&nbsp;</td>
                <td colspan="2" class="py-1 w-2/4">
                    ({{ $con->container_type }}) {{ $con->reference }} FCL CONTAINER
                </td>
            </tr>
        @endforeach
        <tr>
        ...
        <tr>
            ...
            <td class="py-2" colspan="3">{{$containers?->sum('hbl_packages_count')}} PKGS (PERSONAL EFFECTS CARGO)</td>
        </tr>
        <tr>
    ...
    <p>This container / said to contain <strong>{{$containers?->sum('hbl_packages_count')}} packages</strong> personal effects <strong>{{$container->port_of_loading ? $container->port_of_loading : '..........................................................'}}</strong> No objection to removal to Laksiri Seva U.P.B. Bonded Warehouse under Customs Supervision.</p>
    ...
    <div class="flex justify-between items-start gap-6">
        ...
        <!-- Customs Office -->
        <div class="w-1/2 p-4 text-center mt-10">
            <p class="mb-0">..........................................................</p>
