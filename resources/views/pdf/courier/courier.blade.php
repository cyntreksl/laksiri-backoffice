    ...
    @foreach ($packagesChunks as $chunk)
                ...
                <td rowspan="3" style="width: 39%; vertical-align: middle;">
                    @php
                        ...
                        echo '<img style="width: 90%" src="data:image/png;base64,' .
                            base64_encode($generator->getBarcode($hbl->courier_number, $generator::TYPE_CODE_128, 3, 80)) .
                            '">';
                    @endphp
                </td>
        ...
        @if ($loop->last)
            <table class="summary">
                <tr>
                    <td>NO. OF PCS:</td>
                    <td style="text-align: right">{{ $hbl?->packages->count() }}</td>

                    <td>FREIGHT CHARGES</td>
                    <td style="text-align: right">{{ number_format($hbl?->freight_charge, 2) }}</td>

                    <td>DESTI. CHARGES</td>
                    <td style="text-align: right">{{ number_format($hbl?->destination_charge, 2) }}</td>
                </tr>
                <tr>
                    <td>TOTAL VOLUME</td>
                    <td style="text-align: right">{{ round($hbl?->packages->sum('volume'), 3) }}</td>

                    <td>HBL CHARGE</td>
                    <td style="text-align: right">{{ number_format($hbl?->bill_charge, 2) }}</td>

                    <td>GRAND TOTAL</td>
                    <td style="text-align: right">{{ number_format($hbl?->grand_total, 2) }}</td>
                </tr>
                <tr>
                    <td>TOTAL WEIGHT</td>
                    <td style="text-align: right">{{ round($hbl?->packages->sum('weight'), 2) }}</td>

                    <td>HBL VAT CHARGE</td>
                    <td style="text-align: right">-</td>

                    <td>PAID AMOUNT</td>
                    <td style="text-align: right">{{ number_format($hbl?->paid_amount, 2) }}</td>
                </tr>
            </table>
        @endif
