...
class CashSettlementCollection extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'picked_date' => $this->pickup->pickup_date ?? $this->created_at->format('Y-m-d'),
            'weight' => number_format($this->packages->sum('weight'), 2) ?? '-',
            'volume' => number_format($this->packages->sum('volume'), 3) ?? '-',
